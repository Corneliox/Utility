#This Code is an Improvement to tackle palette transparency issues in images
#It checks for images smaller than a specified dimension and upscales them if necessary.
        ...
        for filename in filenames:
            ...
            try:
                ...
                with Image.open(file_path) as img:
                    # NEW: Check for palette transparency and convert to RGBA to fix it
                    if img.mode == 'P' and 'transparency' in img.info:
                        print(f"🎨 CONVERTING: {relative_path} (Palette with transparency to RGBA)")
                        img = img.convert("RGBA")
                        # Save as PNG to ensure transparency is kept, overwriting the original
                        img.save(file_path, "PNG")

                # Re-open for verification and other checks
                with Image.open(file_path) as img:
                    img.verify()
...
def show_results_window(root_window, target_dir, damaged_list, undersized_list, total_checked):
    ...
    if not damaged_list and not undersized_list:
        ...
        # Add a button to close the app since there are no other actions
        ok_button = tk.Button(main_frame, text="OK", command=on_closing)
...
def upscale_images(undersized_list, root_window):
    ...
    for file_path in undersized_list:
        try:
            with Image.open(file_path) as img:
                # Convert to RGBA to handle palette transparency correctly before resizing
                img_to_resize = img.convert("RGBA")
                
                width, height = img_to_resize.size
                ...
                if width < MIN_DIMENSION or height < MIN_DIMENSION:
                    ...
                    img_resized = img_to_resize.resize((new_width, new_height), Image.Resampling.LANCZOS)
